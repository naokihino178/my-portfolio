...
export function getSortedPostsData() {
  ...
  // (/posts下からファイルの名前（200101.mdと200401.md）を取得)
  // path.join(process.cwd(), 'posts') => (nextjs-blog/posts)フォルダの直下にあるファイルのリストを"同期的に"取得
  const fileNames = fs.readdirSync(postsDirectory) // /postsのデータを読み込んで変数fileNamesに代入 (200101.mdと200401.md)
  // => "200101.md と 200401.md"が配列として？代入
  // => [200101.md, 200401.md] ←こんな感じかな？

  // idとdataが返ってくる
  const allPostsData = fileNames.map(fileName => { // fileNames (200101.mdと200401.mdをmapで回す)
    ...
    // (マークダウンファイルを文字列として読む)
    // nextjs-blog/postsと.mdが除かれたfileName（200101と200401）を連結
    const fullPath = path.join(postsDirectory, fileName)
    // => nextjs-blog/posts/200101 と nextjs-blog/posts/200401
    
    // ファイルの中身を読むにはfs.readFileSyncを用いる。
    const fileContents = fs.readFileSync(fullPath, 'utf8')
    // => nextjs-blog/posts/200101 と nextjs-blog/posts/200401の中身を読む
